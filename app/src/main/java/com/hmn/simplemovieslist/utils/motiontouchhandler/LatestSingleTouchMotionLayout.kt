package com.hmn.simplemovieslist.utils.motiontouchhandler
...
class LatestSingleTouchMotionLayout(context: Context, attributeSet: AttributeSet? = null) :
    MotionLayout(context, attributeSet) {
    private val videoToDetchTouch by lazy {
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        if (!touchStart) {
            videoToDetchTouch.getHitRect(viewReact)
            touchStart = viewReact.contains(event.x.toInt(), event.y.toInt())


        }
        return touchStart && super.onTouchEvent(event)
    }
